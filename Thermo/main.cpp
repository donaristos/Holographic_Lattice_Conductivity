...
//  Thermo
//
//  Created by Aristomenis Donos on 11/08/2013.
...
int main(int argc, const char * argv[])
{
	...
	//	Q[6]=Cfunction<double>(testf3,D);
	Q[6]=Cfunction<MyType, MyType>(testf4,D);
	
	ReadFromBin(Q, index, 7, tnp, tnx, "thermodata");
	
	for (My_Int ii=0; ii<7; ++ii) {
		Q[ii].update(D);
	}
	
	long NN(GetLength<MyType>(1, "tempmesh.dat"));
	
	std::cout<< NN << std::endl;
	
	
	for(My_Int ii=0; ii<NN;++ii){
		ReadStamp(ps[0],ii, "tempmesh.dat");
		std::cout << ps[0] << "\t";
	}
	
	std::cout << std::endl;
	
	{My_Int nn(0); MyType w1=0, w2=0, w3=0, tw3=0;
		while (nn<NN) {
		ReadStamp(ps[0],nn, "tempmesh.dat");
		std::cout << "==========" << std::endl;
		std::cout << "mu= " << ps[0] << std::endl;
		...
		{	tw3=w3;
			...
			std::cout << "|1-tw3/w3|= " << fabs(1-(tw3/w3)) << std::endl;}
		while((w2>1.E-3) || fabs(1-(tw3/w3))>1. );
	
//			if (w2<5.E-4) {
	AppendToBin(Q, index, 7, tnp, tnx, "thermodata");
	WriteStamp(ps[0], "tempstamp");
//			}
			++nn;
	}
	}
